...
class IndividualDetailsControllerISpec extends BaseISpec {
  ...
  "Call" must {
    "get with query parameter fields filter for Breathing Space" in {
  ...
  "Call" must {
    "get without query parameter fields (full population)" in {
      val dateOfBirth = LocalDate.now
      val firstForename = "Joe"
      val surname = "Zawinul"
  ...
  "Call" must {
    "get should work even when the given Nino includes the suffix" in {
  ...
  "Call" must {
    "get should return 404(NOT_FOUND) when trying to retrieve details for an unknown Nino" in {
      status(getIndividualDetails(genNino)) shouldBe NOT_FOUND
    }
  }

  "Call" must {
    "get should return 422(UNPROCESSABLE_ENTITY) when fields provides an unknown detail value" in {
