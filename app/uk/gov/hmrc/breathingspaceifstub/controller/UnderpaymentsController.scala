...
class UnderpaymentsController @Inject()(underpaymentsService: UnderpaymentsService, cc: ControllerComponents)(
  ...
  def saveUnderpayments(nino: String, periodId: String): Action[JsValue] = Action.async(parse.json) {
    implicit request =>
      ...
      val maybeUnderpayments: Either[String, List[Underpayment]] = request.body.validate[List[Underpayment]] match {
        case JsSuccess(values, _) => Right(values)
        case JsError(errors) =>
          if (request.body.toString() == "[]") Right(List.empty)
      ...
      val underpayments: List[Underpayment] = if (!parsingError) maybeUnderpayments.right.get else List.empty

      underpaymentsService
        .saveUnderpayments(nino, periodId, underpayments, logger)
        .map(
          _.fold(
            failure =>
